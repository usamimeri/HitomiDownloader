...
class Util:
    ...
    def get_image_infos(gallery_id: str):
        ...
        with requests.get(js_url, headers=headers) as response:
            try:
                data = response.text
                match = re.search(
                    pattern=r'.*?({.*})', string=data).group(1)  # 去除不需要的部分
                images_data = json.loads(match)  # 转为python格式,是字典
                images_data['hash'] = [hash['hash']
                                       for hash in images_data['files']]  # 获取哈希值列表
    ...
    def get_time_info():
        ...
        with requests.get(js_url, headers=headers) as response:
            ...
            try:
                timestamp = re.search(
                    pattern=r'b:\s+\'(\d+)/\'', string=time_data).group(1)
    ...
    def get_image_urls_by_url(url):
        ...
        try:
            ...
            urls = Util.get_image_urls(
                mapping_result, timestamp, subdomains, hash_list)  # 图片的url列表
        except:
...
class HitomiDownloader:
    ...
    def by_url(self, url: str, thread_num: int = 10, delay: float = 0.5):
        ...
        threads = [Thread(target=start_download, args=(url,))
    ...
    def save_image(self, url):
        ...
        if os.path.exists(DIR_NAME):
            if FILE_NAME in os.listdir(DIR_NAME):
                logging.info(f'图片已下载{FILE_NAME}')
                return
